import { Check, Clock, X, Trash } from '@phosphor-icons/react'
...
const STATUS_BADGE = { pending: 'pending', fired: 'pending', done: 'done', missed: 'missed', snoozed: 'snoozed' }
...
export default function NudgeCard({ nudge, onComplete, onSnooze, onDismiss, onDelete }) {
  ...
  return (
    <div className={cn('group flex flex-col gap-2 p-3 rounded-xl border border-border bg-card transition-all', isOver && 'opacity-50')}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
            <span className="text-[11px] text-muted-foreground">{dayjs(nudge.scheduled_for).format('h:mm A')}</span>
            {nudge.recurrence && <Badge variant="outline">{nudge.recurrence.replace(/_/g, ' ')}</Badge>}
            <Badge variant={STATUS_BADGE[nudge.status] ?? 'secondary'} className="ml-auto">{nudge.status}</Badge>
          </div>
        </div>
      </div>
